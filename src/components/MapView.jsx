...
import { MapContainer, TileLayer, Marker, Polyline, Tooltip, ZoomControl, useMapEvents, useMap, Rectangle } from 'react-leaflet'
import L from 'leaflet'
import { Layers, Hand, AlertCircle, Loader2, MapPin } from 'lucide-react'
import RealRoute from './RealRoute'
import { gridToLatLng, getCellBounds } from '../utils/gridHelper'
import { DEFAULT_GRID } from '../utils/algorithms'

// Trung tâm Hà Nội
const HANOI_CENTER = [21.0285, 105.8542]

// Tọa độ thật
const REAL_START = { lat: 21.0298, lng: 105.8360, name: 'Nguyễn Văn Cừ' }
const REAL_GOAL = { lat: 21.0050, lng: 105.8430, name: 'Đại học' }
...
function MapController({ start, goal }) {
 ...
 // Fit bounds bao gồm cả start và goal với padding
...
function ClickHandler({ selectMode, onMapClick, setHoverPos }) {
 useMapEvents({
 click(e) {
 if (selectMode) {
...
function MapView({
 ...
 />
  ...
  {/* Real route theo đường phố - chỉ ở chế độ 3D View */}
  {hasPoints && view === '3d' && (
    <RealRoute
      start={startPoint}
      goal={goalPoint}
  ...
  {/* Grid Search Path - chỉ ở chế độ Grid View */}
  {hasPoints && view === 'grid' && result?.path && result.path.length > 0 && (
    <Polyline
      positions={result.path.map(([pr, pc]) => gridToLatLng(pr, pc))}
      pathOptions={{
        color: '#22d3ee',
        weight: 6,
        opacity: 0.9,
  ...
  {/* Grid Overlay, Obstacles, and Visited Nodes - chỉ khi ở chế độ Grid View */}
  {view === 'grid' && layers.grid && (
    <>
      {/* Grid cells */}
      ...
      {/* Obstacles */}
      {DEFAULT_GRID.obstacles.map(([or, oc], idx) => (
        <Rectangle
          key={`obs-${idx}`}
          bounds={getCellBounds(or, oc)}
          pathOptions={{
            color: '#dc2626',
            weight: 1,
            fillColor: '#dc2626',
      ...
      {/* Visited nodes */}
      {result?.visited && result.visited.map(([vr, vc], idx) => (
        <Rectangle
          key={`vis-${idx}`}
          bounds={getCellBounds(vr, vc)}
          pathOptions={{
            color: '#22d3ee',
            weight: 0.5,
            fillColor: '#22d3ee',
 ...
 {selectMode === 'obstacle' && '🚧 Mode này không khả dụng với routing thật'}
 ...
 <span className="font-semibold">{view === '3d' ? '3D View' : 'Grid View'}</span>
 <span className="text-slate-500">•</span>
 <span>{algorithm === 'astar' ? 'A* Search' : 'Dijkstra'}</span>
 <span className="text-slate-500">•</span>
 <span className="text-cyan-400">🛣️ Real Road Routing</span>
 </div>
 ...
 <button
 onClick={() => {
 // Trigger selectMode qua callback - dùng cách khác: gọi từ parent
 // Tạm thời hướng dẫn user click sidebar
 }}
